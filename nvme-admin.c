...
static uint16_t nvme_cmd_write_amplification(FemuCtrl *n, NvmeCmd *cmd)
{
    uint32_t trans_len;
    uint64_t prp1 = le64_to_cpu(cmd->dptr.prp1);
    uint64_t prp2 = le64_to_cpu(cmd->dptr.prp2);

    trans_len = sizeof(uint64_t);

    uint64_t waf;
    waf = (n->host_write + n->gc_write)*100 / n->host_write;
...
static uint16_t nvme_get_log(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    case NVME_LOG_CMD_WRITE_AMP:
        return nvme_cmd_write_amplification(n, cmd);
    default:
